...
- [ ] **Step 4 (launch checklist — report, do not execute merchant steps):**
  ...
  5. Configure a free/zero-cost shipping rate in Shopify for the membership's shipping zone(s) FIRST, then re-run `scripts/setup-membership.js` to flip existing SUB-* variants to `requires_shipping: true` (final-review C1 — without this, Shopify skips the address step on every configured checkout and auto-redeem's destination gate fails every pair closed). **Order matters:** running `setup-membership.js` before the shipping rate exists hard-blocks the address/shipping step on every membership checkout — including the zero-config purchase that works in production today — for the window between the two steps.
